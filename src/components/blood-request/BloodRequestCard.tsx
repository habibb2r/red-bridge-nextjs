...
const BloodRequestCard = ({ request, onContact }: BloodRequestCardProps) => {
  const getUrgencyColor = (urgency: string) => {
    switch (urgency) {
      ...
      case 'High': return 'bg-orange-500 hover:bg-orange-600 text-white shadow-md';
      case 'Medium': return 'bg-yellow-500 hover:bg-yellow-600 text-white shadow-md';
      case 'Low': return 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-md';
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'fulfilled': return 'bg-blue-100 text-blue-700 border border-blue-200';
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <CardTitle className="text-lg text-gray-800 mb-2">{request.title}</CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-red-500 text-lg">{request.bloodGroup}</span>
          </div>
          <div className="flex items-center space-x-2">
